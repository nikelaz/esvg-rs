...
impl SingleElementPluginTrait for ShapeToPathPlugin {
  fn process(&self, element: &mut Element) -> Result<Element, Box<dyn Error>> {
    ...
    if element_clone.name == "rect" {
      rect_to_path(&mut element_clone);
    }
 
    if element_clone.name == "line" {
      line_to_path(&mut element_clone);       
    }

    if element_clone.name == "polyline" {
      polygon_to_path(&mut element_clone, false);
    }

    if element_clone.name == "polygon" {
      polygon_to_path(&mut element_clone, true);
    }
